...
public class CatalogGUI extends JPanel {
	...
	private JPanel getDataFieldsPanel(ArrayList<DataField> dataFields) {
		...
		for (DataField data : dataFields ){
			JLabel name = new JLabel(data.getName(GourmetCoffeeGUI.isEnglish) + ":");//增加参数
			
			JTextField value = new JTextField(data.getValue(GourmetCoffeeGUI.isEnglish),20);
		...
		return dataFieldsPanel; // REMOVE; USED SO THIS FILE COMPILES
